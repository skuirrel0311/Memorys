            ...
            if (leader.Value == null) {
                if (targetGroup.Value.Count > 0) {
                    for (int i = 0; i < targetGroup.Value.Count; ++i) {
                        ...
                        if (damagable != null) {
                            AddTarget(targetGroup.Value[i].transform, damagable);
                        }
                    }
                } else {
                    ...
                    for (int i = 0; i < foundAttackGroup.Length; ++i) {
                        ...
                        if (damagable != null) {
                            AddTarget(foundAttackGroup[i].transform, damagable);
                        }
                    }
                }
...
#if DEATHMATCH_AI_KIT_PRESENT
                TeamManager.RemoveFromFormation(leader.Value, Owner);
#endif
            }
